...
public class Result<TValue>: Result {
	...
	public TValue? Value {
		get {
			if(HasErrors)
				throw new InvalidOperationException("The value cannot be get from an instance with errors.");
			return value;
		}
		set {
			if(HasErrors)
				throw new InvalidOperationException("The value cannot be set to an instance with errors.");
			this.value = value;
		}
	}
...
public class ResultError: ValueObject, IComparable<ResultError> {
	...
	public ResultError(string? code, string description) {
		if(string.IsNullOrWhiteSpace(description))
			throw new ArgumentException("The error description is invalid.", nameof(description));
		Code = code;
